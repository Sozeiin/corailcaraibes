...
DROP FUNCTION IF EXISTS update_stock_item_purchase_info();
...
BEGIN
  ...
  IF NEW.status IN ('delivered', 'completed') AND 
    ...
    INSERT INTO notifications (
      ...
      COALESCE(NEW.requested_by, NEW.created_by),
      'stock_sync_success',
...
-- 5. Créer une fonction pour forcer la synchronisation des commandes existantes
CREATE OR REPLACE FUNCTION force_sync_delivered_orders()
RETURNS TABLE(order_id uuid, order_number text, items_synced integer, success boolean, error_message text)
AS $$
DECLARE
  order_record RECORD;
  synced_count integer;
BEGIN
  -- Récupérer toutes les commandes livrées qui n'ont pas d'articles liés au stock
  FOR order_record IN 
    SELECT DISTINCT o.id, o.order_number, o.status, o.delivery_date, o.base_id, o.supplier_id, o.requested_by
    FROM orders o
    WHERE o.status IN ('delivered', 'completed')
    AND EXISTS (
      SELECT 1 FROM order_items oi 
      WHERE oi.order_id = o.id AND oi.stock_item_id IS NULL
    )
  LOOP
    BEGIN
      -- Déclencher manuellement la synchronisation
      PERFORM sync_order_to_stock_and_history() FROM (
        SELECT order_record.* as NEW, NULL as OLD
      ) t;
      
      -- Compter les items synchronisés
      SELECT COUNT(*) INTO synced_count
      FROM order_items 
      WHERE order_id = order_record.id AND stock_item_id IS NOT NULL;
