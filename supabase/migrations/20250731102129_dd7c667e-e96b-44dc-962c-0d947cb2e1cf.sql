-- Phase 1 Continued: Fix remaining functions and role security
...
-- CRITICAL: Prevent users from updating their own roles
-- Remove existing conflicting policies and create secure ones
...
WITH CHECK (
  ...
  AND (
    ...
    OR (
      -- Regular users can only update their own records but NOT role or base_id
      OLD.role = NEW.role 
      AND OLD.base_id = NEW.base_id
    )
  )
);

-- Add security event logging for role changes
