-- Fonction pour gérer le workflow automatique lors du scan de stock
...
BEGIN
  ...
  IF NEW.quantity > OLD.quantity THEN
    ...
    FOR order_record IN 
      SELECT DISTINCT o.id, o.order_number, o.requested_by
      ...
      WHERE o.status = 'shipping_antilles'
        ...
        AND (
          ...
          -- Correspondance par similarité de nom (>80%)
          OR similarity(LOWER(oi.product_name), LOWER(NEW.name)) > 0.8
        )
    LOOP
      ...
      INSERT INTO public.workflow_notifications (
        ...
        'auto_completion',
        '✅ Demande d''achat terminée automatiquement',
        'Votre demande d''achat ' || order_record.order_number || ' a été automatiquement marquée comme terminée suite au scan de l''article "' || NEW.name || '" en stock.'
    ...
    -- Si des demandes ont été mises à jour, créer une alerte pour informer l'équipe
    IF matched_count > 0 THEN
      INSERT INTO public.alerts (
        ...
        'workflow',
        'info',
        'Demandes d''achat mises à jour automatiquement',
        matched_count || ' demande(s) d''achat ont été automatiquement marquées comme terminées suite au scan de "' || NEW.name || '".',
        NEW.base_id
      );
    END IF;
  END IF;
...
-- Créer le trigger sur les mises à jour de stock_items
DROP TRIGGER IF EXISTS stock_scan_workflow_trigger ON public.stock_items;
CREATE TRIGGER stock_scan_workflow_trigger
