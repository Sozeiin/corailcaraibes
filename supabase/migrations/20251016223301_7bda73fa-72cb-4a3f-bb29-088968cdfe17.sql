-- Corriger la fonction delete_boat_cascade pour inclure orders et order_items
...
BEGIN
  ...
  DELETE FROM public.messages WHERE topic_id IN (SELECT id FROM public.topics WHERE boat_id = boat_id_param);
  ...
  -- 22. Supprimer order_items (via orders liés au bateau)
  DELETE FROM public.order_items WHERE order_id IN (SELECT id FROM public.orders WHERE boat_id = boat_id_param);
  
  -- 23. Supprimer purchase_workflow_steps (via orders liés au bateau)
  DELETE FROM public.purchase_workflow_steps WHERE order_id IN (SELECT id FROM public.orders WHERE boat_id = boat_id_param);
  
  -- 24. Supprimer workflow_notifications (via orders liés au bateau)
  DELETE FROM public.workflow_notifications WHERE order_id IN (SELECT id FROM public.orders WHERE boat_id = boat_id_param);
  
  -- 25. Supprimer workflow_alerts (via orders liés au bateau)
  DELETE FROM public.workflow_alerts WHERE order_id IN (SELECT id FROM public.orders WHERE boat_id = boat_id_param);
  
  -- 26. Supprimer orders liés au bateau
  DELETE FROM public.orders WHERE boat_id = boat_id_param;
  
  -- 27. Les tables avec CASCADE défini se supprimeront automatiquement:
  -- boat_documents, boat_rentals, preparation_checklist_templates
  
  -- 28. Enfin, supprimer le bateau
