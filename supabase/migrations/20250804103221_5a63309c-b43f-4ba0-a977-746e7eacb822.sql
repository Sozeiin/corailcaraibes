...
USING (
  ...
  AND (get_user_role()::text = 'direction' OR base_id = get_user_base_id())
);
...
USING (
  EXISTS (
    ...
    AND (get_user_role()::text = 'direction' OR pa.base_id = get_user_base_id())
  )
);
...
USING (
  get_user_role() = ANY(ARRAY['direction'::user_role, 'chef_base'::user_role])
  AND (get_user_role()::text = 'direction' OR base_id = get_user_base_id() OR base_id IS NULL)
);
