-- Modifier le trigger handle_stock_scan_workflow pour créer automatiquement l'historique d'achat
...
BEGIN
  ...
  IF NEW.quantity > OLD.quantity THEN
    ...
    LOOP
      ...
      -- Créer l'historique d'achat automatiquement
      IF order_item_record.id IS NOT NULL AND order_item_record.unit_price > 0 THEN
        INSERT INTO public.component_purchase_history (
          ...
          COALESCE((SELECT CAST(COALESCE(metadata->>'warranty_months', '12') AS INTEGER) FROM orders WHERE id = order_record.id), 12),
          CURRENT_DATE,
