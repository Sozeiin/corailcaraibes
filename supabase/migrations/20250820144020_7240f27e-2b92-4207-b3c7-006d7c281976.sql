-- Modifier la contrainte pour permettre les achats de stock général
ALTER TABLE public.component_purchase_history 
DROP CONSTRAINT IF EXISTS check_component_or_sub_component_purchase;

-- Nouvelle contrainte plus flexible : soit on a un composant/sous-composant, soit on a juste un stock_item_id
ALTER TABLE public.component_purchase_history 
ADD CONSTRAINT check_purchase_history_reference 
CHECK (
  (component_id IS NOT NULL OR sub_component_id IS NOT NULL) OR 
  (stock_item_id IS NOT NULL AND component_id IS NULL AND sub_component_id IS NULL)
);

-- Mettre à jour la fonction pour utiliser la table existante
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Créer l'historique d'achat dans la table existante avec stock_item_id seulement
