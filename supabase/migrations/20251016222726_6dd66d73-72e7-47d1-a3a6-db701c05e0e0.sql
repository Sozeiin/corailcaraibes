-- Fonction RPC pour supprimer un bateau et toutes ses données liées en cascade
...
BEGIN
  ...
  -- 13. Supprimer intervention_parts (via interventions)
  ...
  -- 18. Supprimer planning_activities
  DELETE FROM public.planning_activities WHERE boat_id = boat_id_param;
  
  -- 19. Les tables avec CASCADE défini se supprimeront automatiquement:
  -- boat_documents, boat_rentals, maintenance_manuals -> maintenance_manual_tasks, 
  -- scheduled_maintenance, preparation_checklist_templates
  
  -- 20. Enfin, supprimer le bateau
...
-- Ajouter les clés étrangères manquantes avec CASCADE
ALTER TABLE public.boat_components
DROP CONSTRAINT IF EXISTS boat_components_boat_id_fkey,
ADD CONSTRAINT boat_components_boat_id_fkey 
  FOREIGN KEY (boat_id) 
  REFERENCES public.boats(id) 
  ON DELETE CASCADE;

ALTER TABLE public.boat_safety_controls
DROP CONSTRAINT IF EXISTS boat_safety_controls_boat_id_fkey,
ADD CONSTRAINT boat_safety_controls_boat_id_fkey 
  FOREIGN KEY (boat_id) 
  REFERENCES public.boats(id) 
  ON DELETE CASCADE;