...
-- Vérifier les politiques actuelles et les corriger
DROP POLICY IF EXISTS "Direction and chef_base can manage suppliers" ON public.suppliers;

-- Créer des politiques séparées pour plus de clarté
...
-- Politique pour la visualisation (plus permissive)
CREATE POLICY "Users can view suppliers in their base or all if direction" 
ON public.suppliers 
FOR SELECT 
USING (
