-- Correction définitive de la fonction handle_stock_scan_workflow pour éviter l'erreur total_cost
...
BEGIN
  ...
  IF NEW.quantity > OLD.quantity THEN
    ...
    LOOP
      ...
      -- Créer l'historique d'achat automatiquement (SANS total_cost car c'est une colonne générée)
      IF order_item_record.id IS NOT NULL AND order_item_record.unit_price > 0 THEN
        BEGIN
          ...
          ON CONFLICT (stock_item_id, order_id) DO NOTHING; -- Éviter les doublons
        EXCEPTION
          WHEN OTHERS THEN
            -- Log error mais ne pas faire échouer l'opération principale
