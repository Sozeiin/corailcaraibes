-- Fix the component_purchase_history table - remove total_cost from trigger inserts since it's a generated column
DROP TRIGGER IF EXISTS trigger_calculate_intervention_part_total ON component_purchase_history;

-- Recreate the trigger only for intervention_parts table if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'intervention_parts') THEN
        CREATE TRIGGER trigger_calculate_intervention_part_total
            BEFORE INSERT OR UPDATE ON intervention_parts
            FOR EACH ROW EXECUTE FUNCTION calculate_intervention_part_total();
    END IF;
END $$;

-- Update the stock scan workflow function to not include total_cost
