...
export function ProductAutocomplete({
  ...
  const filteredItems = stockItems.filter(item => {
    ...
    return words.every(word => item.name.toLowerCase().includes(word) || item.reference && item.reference.toLowerCase().includes(word) || item.category && item.category.toLowerCase().includes(word));
  }).sort((a, b) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const exactMatches = stockItems.filter(item => item.name.toLowerCase() === newValue.toLowerCase());
  ...
  return <div className="relative">
      <div className="relative">
        ...
        <Input type="text" placeholder={placeholder} value={searchValue} onChange={handleInputChange} onFocus={handleInputFocus} onBlur={handleInputBlur} className="w-16" />
      </div>

      {showResults && filteredItems.length > 0 && <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
          {filteredItems.slice(0, 10).map(item => <div key={item.id} className="px-4 py-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 last:border-b-0" onClick={() => handleSelect(item)}>
      ...
      {showResults && filteredItems.length === 0 && searchValue.trim() && <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg">
          <div className="p-4 text-center">
            <p className="text-sm text-gray-500 mb-3">
              Aucun produit trouvé pour "{searchValue}"
            </p>
            {onCreateNew && <Button variant="outline" size="sm" onClick={handleCreateNew} className="w-full">
