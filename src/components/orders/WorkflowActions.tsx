...
export function WorkflowActions({ order, onOrderUpdate }: WorkflowActionsProps) {
  const { user } = useAuth();
  ...
  const getAvailableActions = () => {
    ...
    if ((user.role === 'direction' || (user.role === 'chef_base' && order.baseId === user.baseId)) && 
        ...
        <AlertDialogContent>
          ...
          {action.requiresNotes && (
            <div className="space-y-2">
              ...
              <Textarea
                id="notes"
                value={action.useRejectionReason ? rejectionReason : notes}
                onChange={(e) => action.useRejectionReason ? setRejectionReason(e.target.value) : setNotes(e.target.value)}
                placeholder={action.useRejectionReason ? 'Expliquez pourquoi cette demande est rejetée...' : 'Ajoutez des notes...'}
                rows={3}
              />
            </div>
          )}
