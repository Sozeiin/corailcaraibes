  ...
  return <div className="overflow-x-auto">
      <Table>
        ...
        <TableBody>
          ...
          return <TableRow key={item.id} className={`hover:bg-muted/50 ${onViewDetails ? 'cursor-pointer' : ''}`} onClick={() => onViewDetails?.(item)}>
                <TableCell className="font-medium min-w-[220px]">
                  <div className="flex items-center gap-3">
                    <div className="flex-shrink-0">
                      <OptimizedImage src={item.photoUrl} alt={item.name} size="sm" className="rounded-md border" fallbackIcon={<Package className="h-8 w-8 text-muted-foreground" />} />
                    </div>
                    <div className="flex flex-col gap-1 min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        {item.quantity <= item.minThreshold && <AlertTriangle className="h-4 w-4 text-orange-500 flex-shrink-0" />}
                ...
                <TableCell className="hidden lg:table-cell">
                  {item.baseName && <Badge variant="secondary" className="text-xs">{item.baseName}</Badge>}
                </TableCell>
                ...
                {canManage && <TableCell className="text-right">
                  ...
                  e.stopPropagation();
                  onDuplicate(item);
                }} title="Dupliquer sur une autre base" className="h-8 w-8 p-0">
