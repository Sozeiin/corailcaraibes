      ...
      const formsWithRelations = await Promise.all(
        (data || []).map(async (form) => {
          const [customerData, boatData, suggestedBoatData] = await Promise.all([
            ...
            form.boat_id ? supabase.from('boats').select('*').eq('id', form.boat_id).single() : Promise.resolve({ data: null }),
            form.suggested_boat_id ? supabase.from('boats').select('*').eq('id', form.suggested_boat_id).single() : Promise.resolve({ data: null }),
          ]);
